...
kruskal.test(PC6 ~ group.1, data = pc_cor)
kruskal.test(PC6 ~ timename, data = pc_cor)
kruskal.test(PC6 ~ timename_genotype, data = pc_cor)

library(FSA)
timename_group.test <- data.frame(sample=d$sample,genotype=d$group.1, temp=d$temp, timename=as.factor(d$timename), PC2=pc_cor$PC2, PC3=pc_cor$PC3, PC5=pc_cor$PC5, PC6=pc_cor$PC6)
...
kruskal.test(PC5 ~ timename, data = timename_group_Amb.test)
kruskal.test(PC6 ~ timename, data = timename_group_Amb.test)
timename_group_Amb_PC5.dunn <- dunnTest(PC5 ~ timename,
              data=timename_group_Amb.test,
              method="bh")    # Can adjust p-values;
                              # See ?p.adjust for options
timename_group_Amb_PC6.dunn <- dunnTest(PC6 ~ timename,
              data=timename_group_Amb.test,
              method="bh")    # Can adjust p-values;
                              # See ?p.adjust for options

write_csv(as.data.frame(timename_group_Amb_PC5.dunn$res), "Output/1a-Pacu-mortality-dunn-AMB_PC5.csv")
write_csv(as.data.frame(timename_group_Amb_PC6.dunn$res), "Output/1a-Pacu-mortality-dunn-AMB_PC6.csv")

##Hot

timename_group_Hot.test <- filter(timename_group.test, temp=="Hot")

kruskal.test(PC5 ~ timename, data = timename_group_Hot.test)
kruskal.test(PC6 ~ timename, data = timename_group_Hot.test)

timename_group_Hot_PC5.dunn <- dunnTest(PC5 ~ timename,
              data=timename_group_Hot.test,
              method="bh")    # Can adjust p-values;
timename_group_Hot_PC6.dunn <- dunnTest(PC6 ~ timename,
              data=timename_group_Hot.test,
              method="bh")    # Can adjust p-values;
                              # See ?p.adjust for options

write_csv(as.data.frame(timename_group_Hot_PC5.dunn$res), "Output/1a-Pacu-mortality-dunn-Hot_PC5.csv")
write_csv(as.data.frame(timename_group_Hot_PC6.dunn$res), "Output/1a-Pacu-mortality-dunn-Hot_PC6.csv")
```
...
ggsave("Output/1a-Pacu-edgeR-triploid-only-DEGs-PCA.pdf", allgenes_PCA)
```
