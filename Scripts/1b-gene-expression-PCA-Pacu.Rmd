...
```{r}
d <- data.frame(Group = DGEdat.cpm$samples$group.1, DGEdat.cpm$samples, name = colnames(DGEdat.cpm$counts)) #make a dataframe containing all plotting info
...
DGEdat_PCcor <- lapply(d[,c(7:10,12,14,16,18)], as.factor)
...
hmTreatment <- subset(DGEdat$samples, select=c("temp"))
...
pdf("Output/1a-Pacu-edgeR-Treatment-DEG-heatmap.pdf", width = 10.5, height = 7)
draw(DEGheatmap)
dev.off()
```
...
id=
percentVar <- pca$sdev^2/sum(pca$sdev^2) #save % variation by PC1 and PC2
d6 <- data.frame(PC1 = pca$x[, 1], PC2 = pca$x[, 2], temp = as.factor(DGEdat.cpm$samples$temp), Group = as.factor(DGEdat.cpm$samples$temp), #make a dataframe containing all plotting info
...
ggsave("Output/1a-Pacu-edgeR-allsamples-DEGs-PCA.pdf", DEG_PCA)
```