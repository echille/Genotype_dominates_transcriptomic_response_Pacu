...
cluster.df <- subset(cluster.df, select=c("gene_id", "contrast", "log2FoldChange"))
...
G2vG6 <- cluster.df %>% filter(contrast == "Group2_v_Group6")
G3vG6 <- cluster.df %>% filter(contrast == "Group3_v_Group6")
TvD.shared <- intersect(G2vG6$gene_id, G3vG6$gene_id)
TvD <- filter(cluster.df, gene_id %in% TvD.shared)

#Between triploid groups
G3vG2 <- filter(cluster.df, contrast == "Group3_v_Group2")
```
...
GO.ref <- right_join(GO.ref, length.df, by="gene_id")
GO.ref$GO_terms <- gsub("-","",GO.ref$GO_terms)
...
C1genes <- as.vector(DEGs1$gene_id)
length(C1genes)
...
C2genes <- as.vector(DEGs2$gene_id)
length(C2genes)
...
C3genes <- as.vector(DEGs3$gene_id)
length(C3genes)
...
C4genes <- as.vector(DEGs4$gene_id)
length(C4genes)
...
C5genes <- as.vector(DEGs5$gene_id)
length(C5genes)
C5genes.matrix=as.integer(GO.ref$gene_id%in%C5genes)
names(C5genes.matrix)=GO.ref$gene_id
length(C5genes.matrix)
```
...
pwf.5<-nullp(DEgenes=C5genes.matrix, id=IDvector, bias.data=lengthVector)
```

Stopping to check that the matrix worked as it should have. Checking the number of DEGs
```{r}
nrow(filter(pwf.1, DEgenes==1))
nrow(filter(pwf.2, DEgenes==1))
nrow(filter(pwf.3, DEgenes==1))
nrow(filter(pwf.4, DEgenes==1))
nrow(filter(pwf.5, DEgenes==1))
...
GOwall.5 <- goseq(pwf.5, IDvector, gene2cat=GO.terms, test.cats=c("GO:CC", "GO:BP", "GO:MF"), method="Wallenius", use_genes_without_cat=TRUE)
```
...
up.GO5<-GOwall.5$category[GOwall.5$over_represented_pvalue<.05]
up.GO5<-data.frame(up.GO5)
colnames(up.GO5) <- c("category")
up.GO5 <- merge(up.GO5, GOwall.5, by="category")
up.GO5 <- up.GO5[order(up.GO5$ontology, up.GO5$over_represented_pvalue, -up.GO5$numDEInCat),]
up.GO5$term <- as.factor(up.GO5$term)
nrow(filter(up.GO5, ontology=="BP")) #number sig BP terms
nrow(filter(up.GO5, ontology=="MF")) #number sig MF terms
nrow(up.GO5)

```
Save significant terms
```{r}
# write.csv(up.GO1, file = "C:\\Users\\19dee\\OneDrive\\Documents\\GitHub\\Triploid-Pacu-Gene-Dosage\\Output\\up.GO1.csv", row.names = FALSE)
# write.csv(up.GO2, file = "C:\\Users\\19dee\\OneDrive\\Documents\\GitHub\\Triploid-Pacu-Gene-Dosage\\Output\\up.GO2.csv", row.names = FALSE)
# write.csv(up.GO3, file = "C:\\Users\\19dee\\OneDrive\\Documents\\GitHub\\Triploid-Pacu-Gene-Dosage\\Output\\up.GO3.csv", row.names = FALSE)
# write.csv(up.GO4, file = "C:\\Users\\19dee\\OneDrive\\Documents\\GitHub\\Triploid-Pacu-Gene-Dosage\\Output\\up.GO4.csv", row.names = FALSE)
# write.csv(up.GO5, file = "C:\\Users\\19dee\\OneDrive\\Documents\\GitHub\\Triploid-Pacu-Gene-Dosage\\Output\\up.GO5.csv", row.names = FALSE)

write.csv(up.GO1, file = "Output/NEW_up.GO1.csv", row.names = FALSE)
write.csv(up.GO2, file = "Output/NEW_up.GO2.csv", row.names = FALSE)
write.csv(up.GO3, file = "Output/NEW_up.GO3.csv", row.names = FALSE)
write.csv(up.GO4, file = "Output/NEW_up.GO4.csv", row.names = FALSE)
write.csv(up.GO5, file = "Output/NEW_up.GO5.csv", row.names = FALSE)
```

## Find associated GOslim terms to streamline interpretation of our very cool results

Load in Rdata, if necessary.
```{r}
#Rdata.2 <- load(file = "Scripts/20210916_R_session.RData")
```

Join our dataframes containing significantly up and down-regulated terms.
```{r}
up.GO1$Cluster <- "Cluster1"
up.GO2$Cluster <- "Cluster2"
up.GO3$Cluster <- "Cluster3"
up.GO4$Cluster <- "Cluster4"
up.GO5$Cluster <- "Cluster5"
all_GO <- bind_rows(up.GO1, up.GO2, up.GO3, up.GO4) #, up.GO5)  #bind rows
str(all_GO)
```
...
```{r}
BPplot <- na.omit(GO.BP) %>% filter(numDEInCat>2) %>% #filter(over_represented_pvalue<0.01) %>% 
...
# ggsave("C:\\Users\\19dee\\OneDrive\\Documents\\GitHub\\Triploid-Pacu-Gene-Dosage\\Output\\GOslimBPplot.pdf", BPplot, width = 15, height = 20, units = c("in"))
ggsave("Output/NEW_GOslimBPplot.pdf", BPplot, width = 15, height = 20, units = c("in"))

MFplot <- GO.MF %>% filter(numDEInCat>2) %>% 
...
#ggsave("C:\\Users\\19dee\\OneDrive\\Documents\\GitHub\\Triploid-Pacu-Gene-Dosage\\Output\\GOslimMFplot.pdf", MFplot, width = 15, height = 20, units = c("in"))
ggsave("Output/NEW_GOslimMFplot.pdf", MFplot, width = 15, height = 20, units = c("in"))

GOfig <- BPplot + MFplot
GOfig <- ggdraw(plot = GOfig) + draw_plot_label(c("a)", "b)"), c(0, 0.35), c(1, 1), size = 15)
#ggsave("C:\\Users\\19dee\\OneDrive\\Documents\\GitHub\\Triploid-Pacu-Gene-Dosage\\Output\\GOslim.pdf", GOfig, width = 20, height = 24, units = c("in"))
ggsave("Output/NEW_GOslim.pdf", GOfig, width = 20, height = 24, units = c("in"))
```
